...
(function (global) {
    ...
    function createMonthTable(data, option) {
        ...
        count = 1;
        //There is no need to worry about the excessive number
        //of td,because tr will not be loaded with extra td.
        while (c <= 6) {
            td = document.createElement("td");
            td.innerHTML = count;
    ...
    // result.month
    // result.date
    // result.today.dayIndex
    // result.today.dayName
    // result.today.dayFullName
    // result.today.monthIndex
    // result.today.monthName
    // result.today.monthNameFull
    // result.today.date
    // result.today.year

    // result.firstDayIndex first day of the month
    // result.firstDayName
    // result.firstDayFullName
    // result.monthIndex
    // result.monthName
    // result.monthNameFull
    // result.totaldays
    // result.targetedDayIndex
    // result.targetedDayName
    // result.targetedDayFullName

    function getCalendar(year, month, date) {

        // example: year:2023 month:1 date:17
        var
            dateObj = new Date(),//Tue Jan 17 2023 18:42:57 GMT+0800 (中国标准时间)
            dateString,
            result = {},
            idx;
        ...
        //today
        result.today = {};
        dateString = dateObj.toString().split(" ");//["Tue", "Jan", "17", "2023", "18:42:57", "GMT+0800", "(中国标准时间)"]
    

        //the index of  ddd formate of the current week date 
        idx = dayName.ddd.indexOf(dateString[0]);//idx :2
        result.today.dayIndex = idx;  //2
        result.today.dayName = dateString[0];//"Tue"
        result.today.dayFullName = dayName.full[idx];//"Tuesday"

        idx = monthName.mmm.indexOf(dateString[1]);//0
        result.today.monthIndex = idx;//0
        result.today.monthName = dateString[1];//'Jan'
        result.today.monthNameFull = monthName.full[idx];//'January'
        
        result.today.date = dateObj.getDate();//17

        result.today.year = dateString[3];//'2023'
        ...
        // console.log(dateString);// ["Sun", "Jan", "01", "2023", "19:21:49", "GMT+0800", "(中国标准时间)"]

        idx = dayName.ddd.indexOf(dateString[0]);//0
        result.firstDayIndex = idx;//0
        result.firstDayName = dateString[0];//'Sun'
        result.firstDayFullName = dayName.full[idx];//'Sunday'

        idx = monthName.mmm.indexOf(dateString[1]);//0
        result.monthIndex = idx;//0
        result.monthName = dateString[1];//'Jan'
        result.monthNameFull = monthName.full[idx];//'January'

        //get total days for the month-year
        dateObj.setFullYear(year);
        dateObj.setMonth(month + 1);
        dateObj.setDate(0);
        //month 2,date 0 = totaldays of month 1
        result.totaldays = dateObj.getDate();//31
        ...
        idx = dayName.ddd.indexOf(dateString[0]);//2
        result.targetedDayIndex = idx;//2
        result.targetedDayName = dateString[0];//'Tue'
        result.targetedDayFullName = dayName.full[idx];//'Tuesday'
    ...
    function onClick() {
        ...
        document.body.onclick = function (e) {
            ...
            // console.log(targetDomObject.classList);
            ...
            if ((targetDomObject) && (targetDomObject.classList) && (targetDomObject.classList.contains("dycalendar-prev-next-btn"))) {
                ...
                // console.log(targetDomObject.parentElement);
                if (btn === "prev") {
                    month = month - 1;
                    if (month < 0) {
                        year = year - 1;
